...
def calculate_iou_overlap(pred_poly, test_data_GDF):
    """Get the best intersection over union for a predicted polygon.
    Adapted from: https://github.com/CosmiQ/solaris/blob/master/solaris/eval/iou.py, but
    keeps index of test_data_GDF
    Arguments
    ...
    for idx, row in precise_matches.iterrows():
        ...
        if pred_poly.is_valid and test_poly.is_valid:
            ...
            gt_idx = -1
        row['iou_score'] = iou_score
        row['overlap_score'] = overlap_score
        row['gt_idx'] = gt_idx
        iou_row_list.append(row)
    iou_GDF = gpd.GeoDataFrame(iou_row_list)
    return iou_GDF

def track_footprint_identifiers_twostep(json_dir, out_dir,
    ...
    for j, f in enumerate(json_files):
        ...
        if j == 0:
            ...
            for pred_idx, pred_row in gdf_now.iterrows():
                ...
                # get iou overlap
                iou_GDF = calculate_iou_overlap(pred_poly, gdf_master_Edit)
                ...
                if not iou_GDF.empty:
                    ...
                    if max_iou_row['iou_score'] >= min_iou:
                        ...
                        max_overlap_row = iou_GDF.loc[iou_GDF['overlap_score'].idxmax(axis=0, skipna=True)]
                        max_area_o = max_overlap_row.geometry.area
                        if max_area_o == 0 or math.isnan(max_area_o):
                            # print("nan area!", max_iou_row, "returning...")
                            raise Exception("\n Nan area!:", max_iou_row, "returning...")
                            return
                        id_match_o = max_overlap_row[id_field]
                        if id_match_o in id_set:
                            print("Already seen id! returning...")
                            raise Exception("\n Already seen id!", id_match, "returning...")
                            return
                        if max_overlap_row['overlap_score']>0:
                            gdf_now.loc[pred_row.name, iou_field] = max_overlap_row['overlap_score']
                            gdf_now.loc[pred_row.name, id_field] = id_match_o
                            # drop  matched polygon in ground truth
                            gdf_master_Edit = gdf_master_Edit.drop(max_overlap_row.name, axis=0)
                            n_matched += 1
                        else:
                            # no match,
                            if super_verbose:
