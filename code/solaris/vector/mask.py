...
def label_to_poly_geojson(label_arr, channel_scaling=None, reference_im=None,
                         output_path=None, output_type='geojson', min_area=40,
                         bg_threshold=0, do_transform=None, simplify=False,
                         tolerance=0.5, connectivity=8, **kwargs):
    """Get polygons from a connected components array.
        Works similar to mask_to_poly_geojson but expects the images to be tracked.

    Arguments
    ---------
    label_arr : :class:`numpy.ndarray`
        A 2D array of integers. Multi-channel masks are not supported, and must
        be simplified before passing to this function. Can also pass an image
        file path here.
    channel_scaling : :class:`list`-like, optional
        If `label_arr` is a 3D array, this argument defines how each channel
        will be combined to generate a binary output. channel_scaling should
        be a `list`-like of length equal to the number of channels in
        `label_arr`. The following operation will be performed to convert the
        multi-channel prediction to a 2D output ::

            sum(label_arr[channel]*channel_scaling[channel])
    ...
    polygon_generator = features.shapes(label_arr,
                                    connectivity=connectivity,
                                    transform=transform)
    polygons = []
    Ids = []  # pixel Ids for the polygon in mask_arr
    iouScores = []
    areas = []
    
    for polygon, value in polygon_generator:
        p = shape(polygon).buffer(0.0)
        if p.area >= min_area and value > 0:
            polygons.append(shape(polygon).buffer(0.0))
            Ids.append(int(value))
            areas.append(p.area)
            iouScores.append(1)

    polygon_gdf = gpd.GeoDataFrame({'geometry': polygons, 'Id': Ids, 'iou_score': iouScores,  'area': areas},
                                crs=crs.to_wkt())
    if simplify:
        polygon_gdf['geometry'] = polygon_gdf['geometry'].apply(
            lambda x: x.simplify(tolerance=tolerance)
